[crosstalk-README.md](https://github.com/user-attachments/files/27123852/crosstalk-README.md)
# 🎵 Crosstalk · 串聯
...
一個屬於你和 Claude 的音樂交換系統。分享歌曲、標記聆聽情境、互相留言，一起累積共同的音樂收藏。
...
* 進入項目的 **SQL Editor**
* 複製 [`supabase/setup.sql`](https://github.com/onlonlonl/crosstalk/blob/main/supabase/setup.sql) 的內容粘貼進去
* 點 **Run**
...
把 [`supabase/edge-function.ts`](https://github.com/onlonlonl/crosstalk/blob/main/supabase/edge-function.ts) 的代碼給 Claude，請他部署 Edge Function，設定 `verify_jwt: false`。
...
把 [`CLAUDE_INSTRUCTIONS.md`](https://github.com/onlonlonl/crosstalk/blob/main/CLAUDE_INSTRUCTIONS.md) 的內容給你的 Claude，記得把 `YOUR_PROJECT_ID` 換成你的 Supabase Project ID。Claude 需要連接 **Supabase MCP** 才能參與。
...
A music exchange system for you and your Claude. Share songs, tag listening contexts, leave comments, and build a shared music collection together.
...
* Copy and paste the contents of [`supabase/setup.sql`](https://github.com/onlonlonl/crosstalk/blob/main/supabase/setup.sql)
* Click **Run**
...
Give Claude the code in [`supabase/edge-function.ts`](https://github.com/onlonlonl/crosstalk/blob/main/supabase/edge-function.ts) and ask to deploy it as an Edge Function with `verify_jwt: false`.
...
Give your Claude the instructions in [`CLAUDE_INSTRUCTIONS.md`](https://github.com/onlonlonl/crosstalk/blob/main/CLAUDE_INSTRUCTIONS.md) — replace `YOUR_PROJECT_ID` with your actual Supabase project ID. Claude needs **Supabase MCP** connected to participate.
...
3. Click **Submit**
4. Tell Claude in chat: "your turn, the theme is \_\_\_"
...
MIT — see [LICENSE](https://github.com/onlonlonl/crosstalk/blob/main/LICENSE)
